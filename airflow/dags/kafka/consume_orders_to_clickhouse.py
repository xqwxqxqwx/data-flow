...
def main() -> None:
        ...
        for msg in consumer:
            ...
            rows.append(
                (
                    int(p["event_id"]),
                    int(p["user_id"]),
                    datetime.fromisoformat(p["order_ts"].replace("Z", "+00:00")),
    ...
    client.command(
        ...
        ENGINE = MergeTree
        ORDER BY (order_ts, event_id)
        """
    )
    client.command("TRUNCATE TABLE kafka_orders_raw")
    client.insert(
        "kafka_orders_raw",
        rows,
        column_names=["event_id", "user_id", "order_ts", "amount", "currency"],
    )
